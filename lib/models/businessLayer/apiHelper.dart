...
class APIHelper {
  ...
  Future<dynamic> getBookingDetail(int id) async {
    try {
      final response = await http.post(
        ...
        headers: await global.getApiHeaders(false),
        body: json.encode({
          "order_id": id, /*"lang": global.languageCode*/
        }),
      );
  ...
  Future<dynamic> signUp(
    ...
    try {
      ...
      print("DATA status------${formData.toString()}");
      ...
      print("Exception statusMessage------->>>>>>>" + e.response.statusMessage);
      print("Exception response------->>>>>>>" + e.response.toString());
    }
  }
